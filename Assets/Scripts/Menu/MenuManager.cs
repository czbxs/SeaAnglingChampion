...
//	public Animator openObject;
	...
	public static bool bShowSpecialOffer = true;
	...
	{		 
		...
		bPopUpVisible = true;
		if(SoundManager.Instance!=null) SoundManager.Instance.Play_PopUpShow(.05f);

		if(activeMenu != "PopUpLevelCompleted"  && activeMenu != "PopUpChestGold" && activeMenu != "PopUpChestTime" && activeMenu != "PopUpChestEmpty" && activeMenu != "PopUpOctopusInk" )
		{ 
			EscapeButtonManager.AddEscapeButonFunction("ClosePopUpMenuEsc",menu.name);
		}

		//if(  Application.loadedLevelName == "HomeScene" &&  menu.name != "PopUpSpecialOffer" )
	}
	...
	public void ClosePopUpMenuEsc(string menuName)
	{
		...
		{
			if(menuName == "PopUpBrokenItem") return;


			if(menuName == "PopUpPause")
			{
				//ClosePopUpMenu(GameObject.Find(menuName));	
				//Gameplay.Instance.bPause = false;
				Gameplay.Instance.ButtonPlayClicked();
				EscapeButtonManager.AddEscapeButonFunction("ButtonPauseClicked");
			}
			else if(menuName == "PopUpShop" && Application.loadedLevelName.StartsWith("Gameplay"))
			{ 
				Camera.main.SendMessage("ButtonCloseShopClicked");
			}
			else 
			{
				ClosePopUpMenu(GameObject.Find(menuName));	
	...
	public bool TestSpecialOffer()
	{ 
		if(Shop.RemoveAds !=2 && bShowSpecialOffer && GameData.Level>1)
		{
			StartCoroutine("ShowSpecOfferDelay");
			bShowSpecialOffer = false;
			return true;
		}
		else  
		{
	...
	IEnumerator ShowSpecOfferDelay()
	{
